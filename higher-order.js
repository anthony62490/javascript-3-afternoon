...
//Code Here
let totalPopulation  = populations.reduce((t,x) => t+x);
...
//Code Here
let myStrongest = monstersInYourPocket.filter(x => (x.hasOwnProperty("CP") && x["CP"] > 200));
...
//Code here
let ordersTotal = orders.map(x => x["price"]*(1+x["tax"]));
...
//Code here
let onlyBobs = purchases.filter(x => x["owner"] === "Bob");
let bobsTotal = onlyBobs.reduce((a,i) => a+i["price"], 0);
//Did this with two variables because it became unreadable as one command
